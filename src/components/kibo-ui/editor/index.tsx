...
// Every language lowlight was built with, alphabetically. Read once: the set is
// fixed at module load, and re-deriving it per render would rebuild a ~190-entry
// array on every keystroke inside a code block.
const CODE_BLOCK_LANGUAGES = lowlight.listLanguages().sort();

/** What an unset `language` attribute is called in the picker. */
const PLAIN_LANGUAGE_LABEL = "Plain text";

// The highlighting palette, kept as one string so the node view and the plain
// HTML rendering (copy/paste, read-only surfaces) style a code block the same.
const CODE_BLOCK_CLASS = cn(
  ...
  // Local edit: the palette above is GitHub *light* only — on the dark
  // near-black canvas those navy/indigo hexes fall well under AA, so
  // every group gets a dark counterpart (GitHub dark's palette).
  ...
  "dark:[&_.hljs-section]:text-[#1f6feb]",
  "dark:[&_.hljs-bullet]:text-[#f2cc60]",
  "dark:[&_.hljs-emphasis]:text-[#c9d1d9]",
  "dark:[&_.hljs-strong]:text-[#c9d1d9]",
  "dark:[&_.hljs-addition]:bg-[#033a16] dark:[&_.hljs-addition]:text-[#aff5b4]",
  "dark:[&_.hljs-deletion]:bg-[#67060c] dark:[&_.hljs-deletion]:text-[#ffdcd7]",
);

/**
 * The language picker that sits in a code block's own top-right corner.
 *
 * Part of the block rather than a floating menu: the language is a property of
 * this block, it has to be readable without putting the caret inside, and a
 * pill hovering outside the editor reads as belonging to the page instead.
 *
 * Searchable, because lowlight ships ~190 grammars and a list that long is
 * unusable as a plain dropdown. "Plain text" is a real choice, not the absence
 * of one — it clears the attribute, which serialises back to a bare ``` fence.
 */
const CodeBlockLanguagePicker = ({
  language,
  onSelect,
}: {
    ...
    <Popover onOpenChange={setOpen} open={open}>
      ...
      <PopoverContent align="end" className="w-52 p-0" sideOffset={4}>
        <Command>
          ...
          <CommandList>
            <CommandEmpty className="py-4 text-muted-foreground text-sm">
              No language found.
            </CommandEmpty>
            <CommandItem
              className="justify-between"
              onSelect={() => choose(null)}
              value={PLAIN_LANGUAGE_LABEL}
            >
              {PLAIN_LANGUAGE_LABEL}
              {language ? null : <CheckIcon size={14} />}
            </CommandItem>
            {CODE_BLOCK_LANGUAGES.map((entry) => (
              ...
              >
                {entry}
                {language === entry ? <CheckIcon size={14} /> : null}
              </CommandItem>
            ))}
          </CommandList>
        </Command>
...
const CodeBlockView = ({ node, updateAttributes, editor }: NodeViewProps) => {
  const language = (node.attrs.language as string | null) ?? null;

  return (
    <NodeViewWrapper className="relative my-2">
      {/* pt-9 keeps the first line clear of the control in the corner. */}
      <pre className={cn(CODE_BLOCK_CLASS, "pt-9")}>
        {/* Explicit generic: NodeViewContent wraps `as` in NoInfer, so the tag
            can't be inferred from the prop. */}
        <NodeViewContent<"code"> as="code" />
      </pre>
      {/* contentEditable={false}: without it ProseMirror treats the control as
          part of the document and the caret can land inside it. */}
...
const codeBlockExtension = CodeBlockLowlight.extend({
  ...
  lowlight,
  HTMLAttributes: { class: CODE_BLOCK_CLASS },
});
...
const EditorSlashMenu = ({ items, editor, range }: EditorSlashMenuProps) => (
  <Command
    className="border-border shadow-pop"
    id="slash-command"
    onKeyDown={(e) => {
      e.stopPropagation();
    }}
  >
    ...
    <CommandList>
      {items.map((item) => (
        ...
        >
          <div className="flex size-9 shrink-0 items-center justify-center rounded-sm border bg-chip">
...
export const EditorProvider = ({
  ...
  const defaultExtensions = [
    StarterKit.configure({
      ...
      horizontalRule: {
        HTMLAttributes: {
          class: cn("mt-4 mb-6 border-border border-t"),
        },
      },
      dropcursor: {
        color: "var(--border)",
    ...
    Slash.configure({
      suggestion: {
        // `signal` is required by @tiptap/suggestion v3 and is forwarded rather
        // than dropped so an in-flight lookup can still be aborted.
        items: async ({ editor, query, signal }) => {
          const items = await defaultSlashSuggestions({
            editor,
            query,
            signal,
          });
    ...
    TableHeader.configure({
      HTMLAttributes: {
        class: cn(
          "relative box-border min-w-[1em] border bg-muted p-1 text-start align-top font-medium font-semibold text-foreground",
        ),
      },
    }),
    TaskList.configure({
    ...
    <TooltipProvider>
      <div className={cn(className, "[&_.ProseMirror-focused]:outline-none")}>
        <TiptapEditorProvider
          // Local edit: caller editorProps are MERGED rather than replaced.
          // `{...props}` below would otherwise drop handleKeyDown and take the
          // slash menu's arrow-key navigation with it, so anything a caller
          // needs to set (aria attributes, for one) has to come through here.
          editorProps={{
            ...editorProps,
...
export const EditorFloatingMenu = ({
  ...
  return (
    <FloatingMenu
      className={cn(
        "flex items-center rounded-lg border border-border bg-popover shadow-float",
        className,
      )}
      editor={editor ?? null}
      {...props}
...
export const EditorBubbleMenu = ({
  ...
  return (
    <BubbleMenu
      className={cn(
        "flex rounded-lg border bg-background p-0.5 shadow-float",
        "[&>*:first-child]:rounded-l-[13px]",
        "[&>*:last-child]:rounded-r-[13px]",
        className,
      )}
    ...
    >
      {children && Array.isArray(children)
        ? children.reduce((acc: ReactNode[], child, index) => {
            ...
            // The index IS the identity here: separators sit between a fixed
            // list of toolbar children and never reorder.
            acc.push(
      ...
      <PopoverContent align="start" className="w-60 p-0" sideOffset={10}>
        <form className="flex p-1" onSubmit={handleSubmit}>
          ...
          {editor.getAttributes("link").href ? (
            <Button
              className="flex h-8 items-center rounded-sm p-1 text-destructive transition-all hover:bg-destructive/10 dark:hover:bg-destructive/15"
              onClick={() => {
...
export const EditorTableGlobalMenu = ({
  ...
  return (
    <div
      className={cn(
        "-translate-x-1/2 absolute flex translate-y-1/2 items-center rounded-full border bg-background shadow-pop",
        {
          hidden: !(left || top),
        },
      )}
...
export const EditorTableColumnMenu = ({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className={cn(
          "-translate-x-1/2 -translate-y-1/2 absolute flex h-4 w-7 overflow-hidden rounded-md border bg-background shadow-pop",
          {
            hidden: !(left || top),
          },
        )}
    ...
    <DropdownMenu>
      <DropdownMenuTrigger
        render={
          <Button
            className={cn(
              "-translate-x-1/2 -translate-y-1/2 absolute flex h-7 w-4 overflow-hidden rounded-md border bg-background shadow-pop",
              {
                hidden: !(left || top),
              },
            )}
...
export const EditorCharacterCount = {
  Characters({ children, className }: EditorCharacterCountProps) {
    ...
    return (
      <div
        className={cn(
          "absolute right-4 bottom-4 rounded-md border bg-background p-2 text-muted-foreground text-sm shadow-card",
          className,
        )}
      >
        {children}
