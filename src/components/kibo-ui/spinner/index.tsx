...
const Bars = ({ size = 24, className, ...props }: SpinnerVariantProps) => (
  <svg
    className={cn(
      // DS: loading/progress indicators use the reserved violet accent when
      // standalone; inside a button they inherit the button's text color
      // (white on primary/gradient, ink on ghost, magenta on destructive) so
      // they stay legible on solid-violet fills.
      "text-primary in-data-[slot=button]:text-current",
      className,
    )}
    height={size}
    viewBox="0 0 24 24"
